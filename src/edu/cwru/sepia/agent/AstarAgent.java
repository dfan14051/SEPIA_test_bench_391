...
public class AstarAgent extends Agent {
    ...
    class MapLocation
    {
        ...
        public MapLocation(int x, int y, MapLocation cameFrom, float cost)
        {
            this.x = x;
            this.y = y;
            //Implemented the variables in the constructor
            this.cameFrom = cameFrom;
            this.cost = cost;
        }
        //Two methods changed so that .contains and .equals works for the object and HashSets
        @Override
        public boolean equals(Object compareTo){
        	return (this.x == ((MapLocation)compareTo).x && this.y == ((MapLocation)compareTo).y);
        }
        ...
        public int hashCode(){
        	String hash = "" + this.x;
        	hash +="000";
        	int tempHash = Integer.parseInt(hash);
        	tempHash += this.y;
        	hash = "" + tempHash;
    ...
    {
    	//Check for the obvious callbacks
    	if(currentPath.isEmpty() || enemyFootmanID==-1)
    		return false;
    	//Compares the next location to the enemy footman's current location
    	MapLocation next = currentPath.peek();
    	if(next.x==state.getUnit(enemyFootmanID).getXPosition()&& next.y==state.getUnit(enemyFootmanID).getYPosition()){
    		return true;
    	}
    ...
    {
    	//Closed list, HashSet for easy and quick .contains checks
    	HashSet<MapLocation> closedlist = new HashSet<MapLocation>();
    	
    	//Priority queue with new comparator 
    	 PriorityQueue<MapLocation> openlist = new PriorityQueue<MapLocation>(4, 
    	            new Comparator<MapLocation>(){
    	                public int compare(MapLocation a, MapLocation b){
    	                	int heurValA = heuristic(a, goal);
    	                	int heurValB = heuristic(b, goal);
    	                    if (heurValA > heurValB) 
    	                    	return 1;
    	                    if (heurValA == heurValB) 
    	                    	return 0;
    	                    return -1;
    	 ...
    	 //Loops through the openlist and stops when the heuristic = 1 (adjacent to TH) or when the openlist is empty
    	 while(!openlist.isEmpty() && heuristic(openlist.peek(), goal) != 1){

    		MapLocation current = openlist.remove();  
    		int xval=current.x;
    		int yval=current.y;
    		int[] coord = new int[]{xval,yval};
    		closedlist.add(current);
    		for(int x=-1; x<=1; x++){
    			for(int y=-1; y<=1; y++){
    				
    				
    	    		//Temp Value, .equals to refer to just x and y, we can define the maplocation once and add it
    	    		MapLocation temp =new MapLocation(x + xval, y + yval, current, current.cost+1);
    	    		boolean xbool= 0 <= x + xval && xExtent > x + xval;
    	    		boolean ybool= 0 <= y + yval && yExtent > y + yval;
    	    		boolean footman = true;
    	    		if(enemyFootmanLoc  !=null)
    	    			footman = !enemyFootmanLoc.equals(temp);
    	    		if(!closedlist.contains(temp) && footman && xbool && ybool && !resourceLocations.contains(temp)){
    					openlist.add(temp);
    				}
    			}
    		}
    	...
    	// return path of moves
        return getMoves(openlist.peek(), start);
    }
    
    //Helper Method that returns the stack of moves by following the path of parents back to the start
    private Stack<MapLocation> getMoves(MapLocation end, MapLocation start){
    	Stack<MapLocation> moves = new Stack<MapLocation>();
    ...
    //Helper method to find heuristic
